...
foreach ($arCompanies as $val){
	...
	$val['img'] = CFile::ShowImage($val['company_logo'],150, 150, 'alt='.$val['company_name']);
...
$APPLICATION->SetPageProperty("title", trim(preg_replace('#[^\w\d\s\.\,\-а-я]+#iu','',$arResult['company']['name'])));
$APPLICATION->SetPageProperty("description", $arResult['company']['seo_description']);
$APPLICATION->SetPageProperty("keywords", "Estelife, Производители препаратов, ".$arResult['company']['name']);
