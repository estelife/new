...
$sTableID = "tbl_estelife_threads_typename_list";
	...
	foreach($arID as $ID){
		if(($ID = IntVal($ID))>0 && $_REQUEST['action']=='delete'){
			try{
				...
				$obQuery->builder()->from('estelife_threads_typename')->filter()
					->_eq('id', $ID);
				$obQuery->delete();
...
while($arRecord=$obResult->GetNext()){
	...
	$arActions[] = array(
		...
		"ACTION"=>$lAdmin->ActionRedirect("estelife_threads_type_edit.php?lang=".LANGUAGE_ID."&ID=$f_ID"),
...
$aMenu[] = array(
	...
	"LINK"=>"estelife_threads_type_edit.php?lang=".LANG,
	"ICON" => "btn_new"
);
