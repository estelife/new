...
$sTableID = "tbl_estelife_apparatus_typename_list";
	...
	foreach($arID as $ID){
		if(($ID = IntVal($ID))>0 && $_REQUEST['action']=='delete'){
			try{
				...
				$obQuery->builder()->from('estelife_apparatus_typename')->filter()
					->_eq('id', $ID);
				$obQuery->delete();
...
$obQuery=$obApp->createQuery();
$obQuery->builder()->from('estelife_apparatus_typename');
...
while($arRecord=$obResult->GetNext()){
	...
	if ($arRecord['type']==1){
		$row->AddViewField("TYPE",'Аппараты');
	}
	...
	$arActions[] = array(
		...
		"ACTION"=>$lAdmin->ActionRedirect("estelife_apparatus_type_edit.php?lang=".LANGUAGE_ID."&ID=$f_ID"),
...
$aMenu[] = array(
	...
	"LINK"=>"estelife_apparatus_type_edit.php?lang=".LANG,
	"ICON" => "btn_new"
);
