...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	try{
		...
		if(!$obPost->blank('save'))
			LocalRedirect('/bitrix/admin/estelife_implants_list.php?lang='.LANGUAGE_ID);
		else
			LocalRedirect('/bitrix/admin/estelife_implants_edit.php?lang='.LANGUAGE_ID.'&ID='.$idPill);
...
?>
	...
	<?php
	$tabControl->EndTab();
	$tabControl->Buttons(array("disabled"=>false, "back_url"=>(strlen($back_url) > 0 ? $back_url : "estelife_implants_list.php?lang=".LANGUAGE_ID)));
	$tabControl->End();
	?>
	</form>
<?php
