...
?>
	...
	<form name="estelife_spec" method="POST" action="<?echo $APPLICATION->GetCurPage()?>" enctype="multipart/form-data">
		...
		<tr class="adm-detail-required-field">
			<td width="40%"><?=GetMessage("ESTELIFE_F_METHOD")?></td>
			<td width="60%">
				<select name="method_id" value="">
					<option value="0"><?=GetMessage('ESTELIFE_NOT_IMPORTANT')?></option>
					<?php foreach($arMethods as $arMethod): ?>
						<option value="<?=$arMethod['id']?>"<?=($arMethod['id']==$arResult['spec']['method_id']) ? ' selected="selected"' : ''?>><?=$arMethod['name']?></option>
					<?php endforeach; ?>
				</select>
			</td>
		</tr>
